...
const FighterCard = ({ corner, stats, searchTerm, setSearchTerm, onSelectFighter, prefillFighter }) => {
  const cornerClass = corner === 'red' ? 'red-corner' : 'blue-corner';
  const cornerTitle = corner === 'red' ? 'RED CORNER' : 'BLUE CORNER';
  const hasStats = stats && Object.keys(stats).length > 0;

  // When prefillFighter changes (from Upcoming Events "Predict →"), auto-select
  useEffect(() => {
    if (prefillFighter) {
      setSearchTerm(prefillFighter);
      onSelectFighter(prefillFighter, corner);
    }
  }, [prefillFighter]);
